...
public class MoveManager : MonoBehaviour
{
	...
	public void PlayerMove(int preRow, int preCol, int nextRow, int nextCol)
	{
		...
		if (PlayerMoveCheck(preRow, preCol, nextRow, nextCol))
		{
			...
			if (_isPullBlockMode && _stageArrayData.StageArray[preRow + (preRow - nextRow), preCol + (preCol - nextCol)] == ConstantForGame.MOVE_BLOCK)
            {
				// ブロックを動かす
                bool blockMoveFlag = BlockMove(preRow + (preRow - nextRow),preCol + (preCol - nextCol), preRow, preCol);

                // ターゲットブロックかつ移動できる移動ブロックだったら
                if (_stageArrayData.TargetData[preRow + (preRow - nextRow), preCol + (preCol - nextCol)] == ConstantForGame.TARGET_AREA && blockMoveFlag)
				{
					// ターゲットクリアカウントを下げる
					_targetCount--;
				}
